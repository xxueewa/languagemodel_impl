...
def decode_oracle(model, indexer, exs, num_exs):
    ...
    """
    increase beam size
    90% of this will be the same as decode_basic
    so how do we cheat? we can use the gold labels to help us -> return the right index
    you will have maybe 2-3 additional lines than decode_basic
    otherwise, the easiest way is just to use a for loop and get the best score
    """
    all_example_preds = []
    num_exs_to_use = min(num_exs, len(exs)) if num_exs > 0 else len(exs)
    ...
    for i in range(0, num_exs_to_use):
        ...
        for j in range(0, beam_size):
            output = pred_indices_to_prediction(output_ids[j][1:], indexer)
            exact_match, recall, total_toks = score_sequence(output, gold_labels)
            if recall > max_score:
                max_score = recall
                one_best = output
        all_example_preds.append(one_best)
...
def decode_fancy(model, indexer, exs, num_exs):
    ...
    everything through model.generate() will be the same as decode_oracle
    requires few more lines than decode_oracle after that
    overall, differences between this and oracle should be minimal, mostly in reranking in the for loop
    """
    all_example_preds = []
    num_exs_to_use = min(num_exs, len(exs)) if num_exs > 0 else len(exs)
    ...
    for i in range(0, num_exs_to_use):
        ...
        beam_size = 10
        output_ids = model.generate(dev_input_tensor, num_beams=beam_size, max_length=65, early_stopping=True, num_return_sequences=beam_size)
        input_const, input_list = get_const_and_str_list(exs[i]['input_ids'], indexer)
        one_best = ""
        max_score = 0
        for j in range(0, beam_size):
            output_const, output_list = get_const_and_str_list(output_ids[j][1:].numpy(), indexer)
            cur_score = list_match(input_list, output_list)
            if output_const == input_const:
                cur_score += 1
            if cur_score > max_score:
                max_score = cur_score
                one_best = pred_indices_to_prediction(output_ids[j][1:], indexer)
        all_example_preds.append(one_best)
    return all_example_preds


def get_const_and_str_list(indices, indexer):
    str_list = []
    for i in indices:
        text = indexer.get_object(i)
        str_list.append(text)
        if text in const_list:
            return text, str_list
    return "", str_list


def list_match(input_list, output_list):
    score = 0
